...
public class ProductesVistaController {
	...
	public static void inicio(ProductesDao pd, BotigaImportExportText ImpExText, BotigaImportExportData ImpExData) {
		...
		do {
			...
			if (opcion == 1) {
				...
				menuImportar(pd, ImpExText, ImpExData);
			} else if (opcion == 7) {
				menuExportar(pd, ImpExText, ImpExData);
			}
		} while (opcion != 0);
			//Guardar datos
			pd.guardarFitxer();
	 ...
	 * 
	 */
	private static void menuImportar(ProductesDao pd, BotigaImportExportText ImpExText, BotigaImportExportData ImpExData) {
		int opcion;
		do {
			// menu
			...
			// segun lo que se escoge, se llama al submenu del controlador que toca
			if (opcion == 1) {
				pd.importarText(ImpExText);
				opcion = 0;
			} else if (opcion == 2) {
				pd.importarData(ImpExData);
				opcion = 0;
			}else if (opcion == 3) {
				System.out.println("Todavia no lo he hecho :(");
				opcion = 0;
			}
		} while (opcion != 0);
	}
	
	private static void menuExportar(ProductesDao pd, BotigaImportExportText ImpExText, BotigaImportExportData ImpExData) {
		int opcion;
		do {
			// menu
			...
			// segun lo que se escoge, se llama al submenu del controlador que toca
			if (opcion == 1) {
				ImpExText.exportar("res/datosExport.txt", pd);
				opcion = 0;
			} else if (opcion == 2) {
				ImpExData.exportar("res/datos3.txt", pd);
				opcion = 0;
			}else if (opcion == 3) {
				System.out.println("Todavia no lo he hecho :(");
				opcion = 0;
